...
class CnInfoReports:
 ...
 #           self.data[ 'category'] = 'category_ndbg_szsh;category_bndbg_szsh;category_yjdbg_szsh;category_sjdbg_szsh'
            self.data[ 'category'] = 'category_rcjy_szsh'
    ...
    def download_hk_report(self, stock_dict: dict, seDate: str) -> None:
        for code in stock_dict:
         ...
         #           if '股权' in announcementTitle or '年报' in announcementTitle or '中期报告' in announcementTitle or '中报' in announcementTitle or '季度报告' in announcementTitle or '季报' in announcementTitle:
                    if '质押' in announcementTitle:
    ...
    def crawl(
        self, stocks: str,
        seDate: str = '2021-11-15~' + str(date.today())) -> None:
...
myreports.crawl('603586,603889,603699,603600,603283,300778,002988,300808,002982,603055,300813,603186,603058,300738,002234,600903,603112,300863,603995,002864,603605,603185,605007,603486,601012,603305,688001,002381,688166' )
